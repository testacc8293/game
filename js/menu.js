Game.Menu = Class.create({
  ...
  constructItem: function (n, cfg) {
    ...
    item.cfg = cfg;
    item.index = n;
    item.action = cfg.choice
      ? this.nextChoice.bind(this, item, true)
      : cfg.action;
  ...
  prevChoice: function (item, wrap) {
    ...
    if (item.cfg.choice) {
      if (item.cfg.chosen > 0) this.setChoice(item, item.cfg.chosen - 1);
      else if (wrap) this.setChoice(item, item.cfg.choice.length - 1);
    }
  },
  ...
  nextChoice: function (item, wrap) {
    ...
    if (item.cfg.choice) {
      ...
      else if (wrap) this.setChoice(item, 0);
    }
  },
  ...
  onfocusitem: function (ev) {
    this.select(this.item(ev));
  },
  ...
  click: function () {
    ...
    if (item && item.action) {
      if (this.cfg.onclick) this.cfg.onclick.call(this.game, item);
      item.action.call(this.game);
    }
  },

  onclick: function (ev) {
    if (this.dom.visible()) this.click(this.item(ev));
  ...
  select: function (obj) {
    if (is.valid(obj)) {
      ...
      var item = is.number(obj)
        ? this.items[Math.min(Math.max(obj, 0), this.items.length - 1)]
        : obj;
      if (selected != item) {
