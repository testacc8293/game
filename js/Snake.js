Snakes = function () {
  ...
  var cfg = {
    ...
    difficulties: [
      ...
      { label: 'Normal', dstep: 0.07, dscore: 1.0 },
    ...
    images: [
      ...
      { id: 'fruit', url: 'images/fruit.png' }
    ],

    sounds: [
      { id: 'menu', name: 'sounds/menu', formats: ['mp3', 'ogg'], loop: true },
      { id: 'game', name: 'sounds/game', formats: ['mp3', 'ogg'], loop: true },
      { id: 'eat', name: 'sounds/eat', formats: ['mp3', 'ogg'], pool: 3 },
      { id: 'die', name: 'sounds/die', formats: ['mp3', 'ogg'] },
      {
        id: 'turn',
        name: 'sounds/turn',
        formats: ['mp3', 'ogg'],
      ...
      { id: 'highscore', name: 'sounds/highscore', formats: ['mp3', 'ogg'] }
    ],
  ...
  var game = Class.create({
    ...
    onenterhighscores: function (event, from, to) {
      if (from !== 'name') this.score.dom.highscores.page.fadein();
    },
    onleavehighscores: function (event, from, to) {
      this.score.dom.highscores.page.fadeout();
    },
    onentername: function (event, from, to) {
      this.score.newHighScore();
    },
    onleavename: function (event, from, to) {
      this.score.save();
    },
    onenterquit: function (event, from, to) {
      this.quitmenu.fadein();
    },
    onleavequit: function (event, from, to) {
    ...
    resetDifficulty: function (n) {
      this.storage.difficulty = Game.Math.minmax(
        is.valid(n) ? n : toInt(this.storage.difficulty, 1),
        0,
    ...
    update: function (dt) {
      if (this.is('game')) {
        ...
        if (
          this.snake.occupies(this.snake.head, true) ||
          ...
          this.score.increase(
            this.fruit.score * this.snake.length * this.difficulty.dscore
          );
    ...
    draw: function (ctx) {
      if (!this.is('loading')) this.render.draw(ctx);
    },

    onclick: function (ev) {
      if (this.can('backToMenu')) if (!is.link(ev.target)) this.backToMenu();
    },

    onfocus: function (ev) {
      if (this.is('menu')) this.menu.focus();
      else if (this.is('quit')) this.quitmenu.focus();
    },
    ...
    unoccupied: function (pos) {
      ...
      while (--max) {
        if (!p.blank()) {
          ...
          p4.y = p.y + 1;
          if (
            !this.court.occupies(p) &&
            !this.snake.occupies(p) &&
            !this.court.occupies(p1) &&
            !this.snake.occupies(p1) &&
            !this.court.occupies(p2) &&
            !this.snake.occupies(p2) &&
            !this.court.occupies(p3) &&
            !this.snake.occupies(p3) &&
            !this.court.occupies(p4) &&
            !this.snake.occupies(p4)
          ) {
            return p;
          }
        }
    ...
    step: function (pos, dir) {
      switch (dir) {
        case DIR.LEFT:
          return new Game.Point(pos.x == 0 ? this.maxX : pos.x - 1, pos.y);
        case DIR.RIGHT:
          return new Game.Point(pos.x == this.maxX ? 0 : pos.x + 1, pos.y);
        case DIR.UP:
          return new Game.Point(pos.x, pos.y == 0 ? this.maxY : pos.y - 1);
        case DIR.DOWN:
          return new Game.Point(pos.x, pos.y == this.maxY ? 0 : pos.y + 1);
      }
    },
    ...
    toggleMute: function (on) {
      ...
      if (mute) {
        ...
        if (this.is('game')) this.playGameMusic();
  ...
  var score = Class.create({
    ...
    set: function (n) {
      this.vset((this.score = Math.floor(n)));
    },
    vset: function (n) {
      this.vscore = Math.floor(n);
      this.redraw = true;
    },
    increase: function (n) {
      this.score = this.score + Math.floor(n);
    },
    format: function (n) {
      return ('0000000' + Math.floor(n)).slice(-6);
    },
    ishigh: function () {
    ...
    update: function (dt) {
      if (this.vscore < this.score) {
        ...
        this.vset(
          Math.min(
            this.score,
            this.vscore + 1 + (this.score - this.vscore) / 10
          )
        );
        if (!wasHigh && this.ishigh()) this.game.playHighScoreFx();
      }
    },
    ...
    draw: function () {
      if (this.redraw) {
        ...
        if (this.ishigh()) this.drawHighScore(this.vscore);
        this.game.dom.main.toggleClassName('highscore', this.ishigh());
        this.redraw = false;
      }
    },

    drawScore: function (n) {
      this.dom.score.current.update(this.format(n));
    },
    drawHighScore: function (n) {
      this.dom.score.high.update(this.format(n));
    },
    ...
    buildHighScoreTable: function (data, editEntry) {
      ...
      for (n = 0; n < data.length; n++) {
        entry = data[n];
        current = entry.name.toUpperCase() === this.name.toUpperCase();
        editing = entry === editEntry;
        klass = editing ? 'editing' : current ? 'current' : '';
        name = $({
          tag: 'span',
          klass: 'name',
          text: editing ? null : entry.name,
          dom: editing ? this.dom.highscores.input : null
        });
    ...
    newHighScore: function () {
      ...
      this.dom.highscores.input.value = entry.name;
    },
    ...
    load: function () {
      ...
      this.highscores =
        JSON.parse(this.game.storage.highscores || 'null') || cfg.highscores;
  ...
  var court = Class.create({
    ...
    reset: function (map) {
      ...
      for (y = 0; y < map.layout.length; y++) {
        ...
        for (x = 0; x < row.length; x++) {
          if (row[x] == 'w') this.walls.push(new Game.Point(x, y));
        }
      }
      var n, wall;
    ...
    occupies: function (pos) {
      var wall,
        n,
        max = this.walls.length;
      for (n = 0; n < max; n++) {
        wall = this.walls[n];
        if (wall.equals(pos)) return true;
      }
      return false;
    }
  });
  ...
  var fruit = Class.create({
    ...
    occupies: function (pos) {
      for (var n = 0; n < this.occupied.length; n++) {
        if (this.occupied[n].equals(pos)) return true;
      }
      return false;
    },
  ...
  var sparkles = Class.create({
    ...
    add: function (pos, options) {
      ...
      var spark = {
        pos: pos,
        image: options.image,
        color: options.color || cfg.colors.particles[options.image],
        opacity: 1.0,
        particles: this.getParticles(cfg.sparkles.each)
      };
      this.all.push(spark);
      ...
      spark.animator.addSubject(
        function (value) {
          this.explode(spark, value);
        }.bind(this)
      );
      spark.animator.play();
    },
    ...
    remove: function (spark) {
      ...
      this.particles = this.particles.concat(spark.particles);
  ...
  var snake = Class.create({
    ...
    reset: function (options) {
      ...
      while (--this.growth) this.increase();
    },
    ...
    move: function (dir) {
      var previous = this.moves.length
  ...
  var render = Class.create({
    ...
    draw: function (ctx) {
      if (!this.parts) this.renderParts();
      ...
      for (n = 0, max = this.game.sparkles.all.length; n < max; ++n) {
        ...
        if (is.valid(spark.image)) {
          ...
          ctx.drawImage(
            this.game.images.fruit,
            spark.image * cfg.fruit.size,
            0,
            cfg.fruit.size,
            cfg.fruit.size,
      ...
      ctx.drawImage(
        this.game.images.fruit,
        this.game.fruit.image * cfg.fruit.size,
        0,
        cfg.fruit.size,
        cfg.fruit.size,
      ...
      ctx.drawImage(
        this.game.images.head,
        this.game.snake.dir * 40,
        0,
        40,
        40,
        this.game.snake.head.x * this.game.dw - this.game.dw / 4,
        this.game.snake.head.y * this.game.dh - this.game.dh / 4,
        this.game.dw + this.game.dw / 2,
        this.game.dh + this.game.dh / 2
      );
      ...
      while ((segment = segment.next))
        this.drawPart(
          ctx,
          segment,
          1 + Math.floor(this.nparts * (n++ / this.game.snake.length)),
          toInt(segment.corner)
        );
      ...
      for (n = 0, max = this.game.sparkles.all.length; n < max; n++) {
        ...
        for (p = 0; p < spark.particles.length; p++) {
          ...
          ctx.fillRect(
            (spark.pos.x + particle.x + 0.5) * this.game.dw,
            (spark.pos.y + particle.y + 0.5) * this.game.dh,
            particle.size * (this.game.dw / 2),
            particle.size * (this.game.dh / 2)
          );
        }
      }
    ...
    renderParts: function () {
      this.nparts = 100;
      this.parts = Game.renderToCanvas(
        (1 + this.nparts) * this.game.dw,
        6 * this.game.dh,
        function (ctx) {
          var n, percent;
