StateMachine = {
  ...
  create: function (cfg, target) {
    var initial =
      typeof cfg.initial == 'string' ? { state: cfg.initial } : cfg.initial;
    ...
    return function () {
      if (this.transition)
        throw (
          'event ' +
          name +
          ' innapropriate because previous transition did not complete'
        );

      if (this.cannot(name))
        throw (
          'event ' + name + ' innapropriate in current state ' + this.current
        );
      ...
      if (this.current != to) {
        ...
        if (
          false !== StateMachine.leaveState.call(this, name, from, to, args)
        ) {
